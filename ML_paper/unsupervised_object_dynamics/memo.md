...
各特徴マップは正規化され、特徴マップの空間的期待値(おそらくウエイトが大きい座標の平均をとっていると思われる)を元に(x,y)座標の表現となる。この処理が提案手法のミソである。通常は特徴マップを平滑化するだけでもキーマップの情報は落ちないのだが、画像の座標空間上に落とし込むことでオブジェクトの動きを直感的にトラッキングする。
...
アクション認識のパフォーマンスを試すため、Human3.6Mのテストデータを使ったアクション認識タスクの精度を比較した。その結果、fig8の結果が得られた。



* 実線：最も高頻度で出現するアクションだけを出力するモデルのaccuracy
* 波線：正しいキーポイントの座標から推定するモデルのaccuracy
* 「sup」がついている条件は提案モデルに教師データを与えた場合の結果である。

DeepMind Control Suite(連続値をアクションとした強化学習環境)を使った報酬の推定を行った。画像と報酬のデータセットはDMCSの環境上でランダムな行動をとって集め、提案モデルの入力にキーポイントとアクションの連続値を設定し、画像とアクションから得られる未来の報酬を予測する。結果をfig9に示す。

* 赤線:画像の特徴表現を画像座標上に落とし込んだバージョン
* 青線:画像の特徴表現を平滑化しただけのバージョン
* どのプロットも時間が経つにつれ、エラーが増大していることから正常に予測ができていると言える。
* どの結果も赤線の方がエラー値が少ないことから、キーポイントを画像座標内に変換するアイデアは効果的であったと言える。
